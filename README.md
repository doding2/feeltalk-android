...
- 성인 대상 서비스 특성상 휴대폰 본인 인증을 필수 단계로 포함하고 있으며 인증이 완료되면 화면을 전환하여 다음 단계로 이동합니다.
- 본인 인증 이후에는 닉네임을 설정하고 커플 서비스 특성을 반영한 초대 코드 기반 커플 연결을 제공합니다.
...
- 완료된 챌린지는 히스토리 형태로 확인할 수 있어 커플 간의 공동 경험 기록으로 확인 가능하도록 설계했습니다.

### 5. 화면잠금
![화면잠금 플로우](./image/FeelTalk_LockScreen_Flow.png)
- 화면 잠금은 커플 서비스 특성상 발생할 수 있는 민감 정보 노출을 방지하기 위한 보안 기능입니다.
- 잠금 활성화 시, 앱 진입 때 비밀번호 입력을 요구하여 개인정보를 보호합니다.
