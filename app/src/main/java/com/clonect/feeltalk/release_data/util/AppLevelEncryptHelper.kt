package com.clonect.feeltalk.release_data.util
...
import com.clonect.feeltalk.mvp_data.utils.fromByteArray
import com.clonect.feeltalk.mvp_data.utils.toByteArrayFeelTalk
...
class AppLevelEncryptHelper(
    ...
    fun decrypt(name: String, digest: String): String {
        return try {
            ...
            val cipher = Cipher.getInstance(Constants.APP_LEVEL_CIPHER_ALGORITHM)
    ...
    private fun getAppLevelKey(): SecretKey {
        val keyStore = KeyStore.getInstance(Constants.APP_LEVEL_KEY_PROVIDER)
        keyStore.load(null)

        val secretKeyEntry: KeyStore.SecretKeyEntry? = keyStore.getEntry(Constants.APP_LEVEL_KEY_ALIAS, null) as? KeyStore.SecretKeyEntry
    ...
    private fun generateAppLevelKey(): SecretKey {
        val keyGen = KeyGenerator.getInstance(KeyProperties.KEY_ALGORITHM_AES, Constants.APP_LEVEL_KEY_PROVIDER)
        keyGen.init(
            KeyGenParameterSpec
                .Builder(
                    Constants.APP_LEVEL_KEY_ALIAS,
