package com.clonect.feeltalk.new_data.repository.challenge.paging

import androidx.paging.PagingSource
import androidx.paging.PagingState
import com.clonect.feeltalk.common.Resource
import com.clonect.feeltalk.new_data.mapper.toChallengeList
...
class OngoingChallengePagingSource(
    ...
    override fun getRefreshKey(state: PagingState<Long, Challenge>): Long? {
        val page: Long? = runBlocking(Dispatchers.IO) {
            try {
                val result = getLastOngoingPageNo()
                result.pageNo
            } catch (e: CancellationException) {
                throw e
            } catch (e: Exception) {
                infoLog("ongoing challenge paging error in getRefreshKey(): ${e.localizedMessage}\n${e.stackTrace.joinToString("\n")}")
                null
            }
        }
        return page
    ...
    override suspend fun load(params: LoadParams<Long>): LoadResult<Long, Challenge> {
        return try {
            val pageKey = params.key
                ?: getLastOngoingPageNo().pageNo

            val result = getOngoingList(pageKey).toChallengeList()
            LoadResult.Page(
                data = result,
                prevKey = null,
            ...
            infoLog("ongoing challenge paging error in load(): ${e.localizedMessage}\n${e.stackTrace.joinToString("\n")}")
            LoadResult.Error(e)
        }
    }
    ...
    private suspend fun getLastOngoingPageNo(): LastChallengePageNoDto {
        return remoteDataSource.getLastOngoingChallengePageNo(getAccessToken())
    }

    private suspend fun getOngoingList(pageNo: Long): ChallengeListDto {
        return remoteDataSource.getOngoingChallengeList(getAccessToken(), pageNo)
    }
}